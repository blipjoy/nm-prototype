...
window.cm = (function cm() {
    ...
    var DebugObject = Object.extend({
        ...
        drawSegment : function drawSegment(p, a, b, r) {
            ...
            var da = {
                x : a.x - self.viewport.x + p.x,
                y : height - a.y - self.viewport.y - p.y
            };
            var db = {
                x : b.x - self.viewport.x + p.x,
                y : height - b.y - self.viewport.y - p.y
            };
            ...
            // This superfluous moveTo() is required. *sigh*
            // See: http://goo.gl/XeiUn
            self.context.moveTo(db.x, db.y);
    ...
    me.game.addEntity = function game_addEntity(entity) {
        ...
        if (entity.points) {
            ...
            if (entity.isPolygon) {
                entity.shape = addShape(new cp.PolyShape(entity.body, verts, cp.vzero));
            }
            else {
                len = verts.length - 2;
                entity.shapes = []
                for (var i = 0, a, b; i < len; i += 2) {
                    ...
                    // Default radius to 5
                    shape = addShape(new cp.SegmentShape(entity.body, a, b, 5));
        ...
        entity.body.eachShape(function eachShape(shape) {
            shape.setLayers(c.LAYER_WALL);
        });
